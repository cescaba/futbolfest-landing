<?php
/**
 * Registration anti-abuse guards for Futbol Fest.
 *
...
function futbolfest_registro_guard_rate_limit() {
	...
	foreach ( $limits as $name => $limit ) {
		...
		if ( $count >= $limit['max'] ) {
			wp_send_json_error(
				array( 'message' => 'Demasiados registros desde esta red. Intenta nuevamente mas tarde.' ),
				429
			);
		}
...
function futbolfest_registro_guard_submit_time() {
	...
	if ( ! $rendered_at || $rendered_at > $now || ( $now - $rendered_at ) < FUTBOLFEST_REGISTRO_MIN_SUBMIT_SECONDS ) {
		wp_send_json_error(
			array( 'message' => 'No pudimos validar el formulario. Intentalo nuevamente.' ),
			400
		);
	}
}
